...
function preprocessMarkdown(markdownText) {
  ...
  let processedText = markdownText.replace(
    /:::danger(?:\\\[([^\\\]]*?)\\\])?/g,
    (match, title) => {
      // Si un titre est spécifié, retirer les échappements et l'utiliser; sinon, utiliser "Danger"
  ...
  processedText = processedText.replace(
    ...
    // Traitement des balises :::attention
    processedText = processedText.replace(
      /:::attention(?:\\\[([^\\\]]*?)\\\])?/g,
      (match, title) => {
  ...
  processedText = processedText.replace(
   ...
   // Traitement des balises :::remarque
   processedText = processedText.replace(
    /:::remarque(?:\\\[([^\\\]]*?)\\\])?/g,
    (match, title) => {
